    ...
    const ImVec2& outer_size, float inner_width)
{
    ...
    const ImVec2 actual_outer_size = ImTrunc(CalcItemSize(outer_size, ImMax(avail_size.x, 1.0f),
...
static inline ImGuiSortDirection TableGetColumnAvailSortDirection(ImGuiTableColumn* column, int n)
{
    ...
    return static_cast<ImGuiSortDirection>((column->SortDirectionsAvailList >> (n << 1)) & 0x03);
}
...
void ImGui::TableSortSpecsBuild(ImGuiTable* table)
{
    ...
    if (dirty && sort_specs != nullptr)
        for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
        {
            ...
            sort_spec->SortDirection = static_cast<ImGuiSortDirection>(column->SortDirection);
        }
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        if (IM_BITARRAY_TESTBIT(table->EnabledMaskByDisplayOrder, order_n))
        {
            ...
            ImGuiTableHeaderData request = {
                static_cast<ImGuiTableColumnIdx>(column_n), col_text, col_header_bg,
...
static constexpr float COLUMNS_HIT_RECT_HALF_THICKNESS = 4.0f;
