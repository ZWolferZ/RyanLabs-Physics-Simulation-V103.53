...
static ImVec2 InputTextCalcTextSize(ImGuiContext* ctx, const char* text_begin, const char* text_end,
    const char** remaining = nullptr, ImVec2* out_offset = nullptr,
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (hovered)
    {
        ...
        const bool mods_ok = !(flags & ImGuiButtonFlags_NoKeyModsAllowed) || (!g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.
            KeyAlt);
        if (mods_ok)
        {
...
void ImGui::Scrollbar(ImGuiAxis axis)
{
    ...
    ScrollbarEx(bb, id, axis, &scroll, size_visible, size_contents, rounding_corners);
    ...
    ImS64 size_contents_v, ImDrawFlags draw_rounding_flags)
{
    ...
    const ImS64 win_size_v = ImMax(ImMax(size_contents_v, size_visible_v), static_cast<ImS64>(1));
    const float grab_h_pixels = ImClamp(
        scrollbar_size_v * (static_cast<float>(size_visible_v) / static_cast<float>(win_size_v)), style.GrabMinSize,
        scrollbar_size_v);
    ...
    const ImS64 scroll_max = ImMax(static_cast<ImS64>(1), size_contents_v - size_visible_v);
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f)
    {
        ...
        if (g.ActiveIdIsJustActivated)
        {
            ...
            const bool scroll_to_clicked_location = (g.IO.ConfigScrollbarScrollByPage == false || g.IO.KeyShift ||
                held_dir == 0);
        ...
        else
        {
            ...
            if (IsMouseClicked(ImGuiMouseButton_Left, ImGuiInputFlags_Repeat) && held_dir == g.ScrollbarSeekMode)
            {
                ...
                *p_scroll_v = ImClamp(*p_scroll_v + static_cast<ImS64>(page_dir * size_visible_v),
                    static_cast<ImS64>(0), scroll_max);
            }
        }
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    if (is_indeterminate)
    {
        constexpr float fill_width_n = 0.2f;
...
void ImGui::DataTypeApplyOp(ImGuiDataType data_type, int op, void* output, const void* arg1, const void* arg2)
{
    ...
    case ImGuiDataType_S8:
        if (op == '+')
        {
            *static_cast<ImS8*>(output) = ImAddClampOverflow(*static_cast<const ImS8*>(arg1),
                *static_cast<const ImS8*>(arg2), IM_S8_MIN, IM_S8_MAX);
        }
        if (op == '-')
        {
    ...
    case ImGuiDataType_U8:
        if (op == '+')
        {
            *static_cast<ImU8*>(output) = ImAddClampOverflow(*static_cast<const ImU8*>(arg1),
                *static_cast<const ImU8*>(arg2), IM_U8_MIN, IM_U8_MAX);
        }
        if (op == '-')
        {
    ...
    case ImGuiDataType_S16:
        if (op == '+')
        {
            *static_cast<ImS16*>(output) = ImAddClampOverflow(*static_cast<const ImS16*>(arg1),
                *static_cast<const ImS16*>(arg2), IM_S16_MIN, IM_S16_MAX);
        }
        if (op == '-')
        {
    ...
    case ImGuiDataType_U16:
        if (op == '+')
        {
            *static_cast<ImU16*>(output) = ImAddClampOverflow(*static_cast<const ImU16*>(arg1),
                *static_cast<const ImU16*>(arg2), IM_U16_MIN, IM_U16_MAX);
        }
        if (op == '-')
        {
    ...
    case ImGuiDataType_S32:
        if (op == '+')
        {
            *static_cast<ImS32*>(output) = ImAddClampOverflow(*static_cast<const ImS32*>(arg1),
                *static_cast<const ImS32*>(arg2), IM_S32_MIN, IM_S32_MAX);
        }
        if (op == '-')
        {
    ...
    case ImGuiDataType_U32:
        if (op == '+')
        {
            *static_cast<ImU32*>(output) = ImAddClampOverflow(*static_cast<const ImU32*>(arg1),
                *static_cast<const ImU32*>(arg2), IM_U32_MIN, IM_U32_MAX);
        }
        if (op == '-')
        {
    ...
    case ImGuiDataType_S64:
        if (op == '+')
        {
            *static_cast<ImS64*>(output) = ImAddClampOverflow(*static_cast<const ImS64*>(arg1),
                *static_cast<const ImS64*>(arg2), IM_S64_MIN, IM_S64_MAX);
        }
        if (op == '-')
        {
    ...
    case ImGuiDataType_U64:
        if (op == '+')
        {
            *static_cast<ImU64*>(output) = ImAddClampOverflow(*static_cast<const ImU64*>(arg1),
                *static_cast<const ImU64*>(arg2), IM_U64_MIN, IM_U64_MAX);
        }
        if (op == '-')
        {
    ...
    case ImGuiDataType_Float:
        if (op == '+')
        {
            *static_cast<float*>(output) = *static_cast<const float*>(arg1) + *static_cast<const float*>(arg2);
        }
        if (op == '-')
        {
            *static_cast<float*>(output) = *static_cast<const float*>(arg1) - *static_cast<const float*>(arg2);
    ...
    case ImGuiDataType_Double:
        if (op == '+')
        {
            *static_cast<double*>(output) = *static_cast<const double*>(arg1) + *static_cast<const double*>(arg2);
        }
        if (op == '-')
        {
            *static_cast<double*>(output) = *static_cast<const double*>(arg1) - *static_cast<const double*>(arg2);
...
int ImGui::DataTypeCompare(ImGuiDataType data_type, const void* arg_1, const void* arg_2)
{
    ...
    case ImGuiDataType_Float: return DataTypeCompareT<float>(static_cast<const float*>(arg_1),
        static_cast<const float*>(arg_2));
    case ImGuiDataType_Double: return DataTypeCompareT<double>(static_cast<const double*>(arg_1),
        static_cast<const double*>(arg_2));
    case ImGuiDataType_COUNT: break;
    }
    IM_ASSERT(0);
    return 0;
...
bool ImGui::DataTypeClamp(ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max)
{
    switch (data_type)
    {
    case ImGuiDataType_S8: return DataTypeClampT<ImS8>(static_cast<ImS8*>(p_data), static_cast<const ImS8*>(p_min),
        static_cast<const ImS8*>(p_max));
    case ImGuiDataType_U8: return DataTypeClampT<ImU8>(static_cast<ImU8*>(p_data), static_cast<const ImU8*>(p_min),
        static_cast<const ImU8*>(p_max));
    case ImGuiDataType_S16: return DataTypeClampT<ImS16>(static_cast<ImS16*>(p_data), static_cast<const ImS16*>(p_min),
        static_cast<const ImS16*>(p_max));
    case ImGuiDataType_U16: return DataTypeClampT<ImU16>(static_cast<ImU16*>(p_data), static_cast<const ImU16*>(p_min),
        static_cast<const ImU16*>(p_max));
    case ImGuiDataType_S32: return DataTypeClampT<ImS32>(static_cast<ImS32*>(p_data), static_cast<const ImS32*>(p_min),
        static_cast<const ImS32*>(p_max));
    case ImGuiDataType_U32: return DataTypeClampT<ImU32>(static_cast<ImU32*>(p_data), static_cast<const ImU32*>(p_min),
        static_cast<const ImU32*>(p_max));
    case ImGuiDataType_S64: return DataTypeClampT<ImS64>(static_cast<ImS64*>(p_data), static_cast<const ImS64*>(p_min),
        static_cast<const ImS64*>(p_max));
    case ImGuiDataType_U64: return DataTypeClampT<ImU64>(static_cast<ImU64*>(p_data), static_cast<const ImU64*>(p_min),
    ...
    case ImGuiDataType_Double: return DataTypeClampT<double>(static_cast<double*>(p_data),
    ...
    const char* format, ImGuiSliderFlags flags)
{
    ...
    const bool is_bounded = (v_min < v_max) || ((v_min == v_max) && (v_min != 0.0f || (flags &
    ...
    const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    case ImGuiDataType_S16:
    {
        ...
        bool r = DragBehaviorT<ImS32, ImS32, float>(ImGuiDataType_S32, &v32, v_speed,
            ...
            p_max ? *static_cast<const ImS16*>(p_max) : IM_S16_MAX, format,
            flags);
    ...
    case ImGuiDataType_U16:
    {
        ...
        bool r = DragBehaviorT<ImU32, ImS32, float>(ImGuiDataType_U32, &v32, v_speed,
            ...
            p_max ? *static_cast<const ImU16*>(p_max) : IM_U16_MAX, format,
            flags);
        if (r) *static_cast<ImU16*>(p_v) = static_cast<ImU16>(v32);
        return r;
    }
    case ImGuiDataType_S32: return DragBehaviorT<ImS32, ImS32, float>(data_type, static_cast<ImS32*>(p_v), v_speed,
        p_min
        ? *static_cast<const ImS32*>(p_min)
        : IM_S32_MIN,
        p_max
        ? *static_cast<const ImS32*>(p_max)
        : IM_S32_MAX, format, flags);
    case ImGuiDataType_U32: return DragBehaviorT<ImU32, ImS32, float>(data_type, static_cast<ImU32*>(p_v), v_speed,
        p_min
        ? *static_cast<const ImU32*>(p_min)
        : IM_U32_MIN,
        p_max
        ? *static_cast<const ImU32*>(p_max)
        : IM_U32_MAX, format, flags);
    case ImGuiDataType_S64: return DragBehaviorT<ImS64, ImS64, double>(data_type, static_cast<ImS64*>(p_v), v_speed,
        p_min
        ? *static_cast<const ImS64*>(p_min)
        : IM_S64_MIN,
        p_max
        ? *static_cast<const ImS64*>(p_max)
        : IM_S64_MAX, format, flags);
    case ImGuiDataType_U64: return DragBehaviorT<ImU64, ImS64, double>(data_type, static_cast<ImU64*>(p_v), v_speed,
        p_min
        ? *static_cast<const ImU64*>(p_min)
        : IM_U64_MIN,
        p_max
        ? *static_cast<const ImU64*>(p_max)
        : IM_U64_MAX, format, flags);
    case ImGuiDataType_Float: return DragBehaviorT<float, float, float>(data_type, static_cast<float*>(p_v), v_speed,
        p_min
        ? *static_cast<const float*>(p_min)
        : -FLT_MAX,
        p_max
        ? *static_cast<const float*>(p_max)
        : FLT_MAX, format, flags);
    case ImGuiDataType_Double: return DragBehaviorT<double, double, double>(
        data_type, static_cast<double*>(p_v), v_speed, p_min ? *static_cast<const double*>(p_min) : -DBL_MAX,
    ...
    const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (temp_input_is_active)
    {
        ...
        {
            const int clamp_range_dir = (p_min != nullptr && p_max != nullptr)
        ...
        return TempInputScalar(frame_bb, id, label, data_type, p_data, format, clamp_enabled ? p_min : nullptr,
    ...
    float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    if (is_logarithmic)
    {
        ...
        {
            float zero_point_center = (-static_cast<float>(v_min)) / (static_cast<float>(v_max) - static_cast<float>(
                v_min));
    ...
    if (is_logarithmic)
    {
        ...
        if ((v_min * v_max) < 0.0f) // Range crosses zero, so we have to do this in two parts
        {
            float zero_point_center = (-static_cast<float>(ImMin(v_min, v_max))) / ImAbs(
                static_cast<float>(v_max) - static_cast<float>(v_min)); // The zero point in parametric space
            float zero_point_snap_L = zero_point_center - zero_deadzone_halfsize;
            float zero_point_snap_R = zero_point_center + zero_deadzone_halfsize;
    ...
    const void* p_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    case ImGuiDataType_S8:
    {
        ...
        bool r = SliderBehaviorT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32,
            ...
            *static_cast<const ImS8*>(p_max), format, flags, out_grab_bb);
        if (r) *static_cast<ImS8*>(p_v) = static_cast<ImS8>(v32);
        return r;
    }
    case ImGuiDataType_U8:
    {
        ...
        bool r = SliderBehaviorT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32,
            ...
            *static_cast<const ImU8*>(p_max), format, flags, out_grab_bb);
        if (r) *static_cast<ImU8*>(p_v) = static_cast<ImU8>(v32);
        return r;
    }
    case ImGuiDataType_S16:
    ...
    case ImGuiDataType_S32:
        IM_ASSERT(
            *static_cast<const ImS32*>(p_min) >= IM_S32_MIN / 2 && *static_cast<const ImS32*>(p_max) <= IM_S32_MAX / 2);
    ...
    case ImGuiDataType_S64:
        IM_ASSERT(
            *static_cast<const ImS64*>(p_min) >= IM_S64_MIN / 2 && *static_cast<const ImS64*>(p_max) <= IM_S64_MAX / 2);
    ...
    case ImGuiDataType_Double:
        ...
        return SliderBehaviorT<double, double, double>(bb, id, data_type, static_cast<double*>(p_v),
            *static_cast<const double*>(p_min),
            *static_cast<const double*>(p_max), format, flags, out_grab_bb);
    case ImGuiDataType_COUNT: break;
    }
    IM_ASSERT(0);
    return false;
...
const char* ImParseFormatFindEnd(const char* fmt)
{
    ...
    constexpr unsigned int ignored_lowercase_mask = (1 << ('h' - 'a')) | (1 << ('j' - 'a')) | (1 << ('l' - 'a')) | (1 <<
        ('t' - 'a')) | (1 << ('w' - 'a')) | (1 << ('z' - 'a'));
    ...
    const char** remaining, ImVec2* out_offset, bool stop_on_new_line)
{
    ...
    while (s < text_end)
    {
        ...
        const float char_width = (static_cast<int>(c) < font->IndexAdvanceX.Size
            ? font->IndexAdvanceX.Data[c]
            : font->FallbackAdvanceX) * scale;
...
namespace ImStb
{
    ...
    static float STB_TEXTEDIT_GETWIDTH(ImGuiInputTextState* obj, int line_start_idx, int char_idx)
    {
        ...
        if (static_cast<ImWchar>(c) == '\n') return IMSTB_TEXTEDIT_GETWIDTH_NEWLINE;
        ImGuiContext& g = *obj->Ctx;
        return g.Font->GetCharAdvance(static_cast<ImWchar>(c)) * g.FontScale;
    }
    ...
    static void STB_TEXTEDIT_LAYOUTROW(StbTexteditRow* r, ImGuiInputTextState* obj, int line_start_idx)
    {
        ...
        const ImVec2 size = InputTextCalcTextSize(obj->Ctx, text + line_start_idx, text + obj->TextLen, &text_remaining,
            nullptr, true);
        r->x0 = 0.0f;
        r->x1 = size.x;
...
#define IMSTB_TEXTEDIT_GETPREVCHARINDEX  IMSTB_TEXTEDIT_GETPREVCHARINDEX_IMPL
    ...
    static int IMSTB_TEXTEDIT_GETPREVCHARINDEX_IMPL(ImGuiInputTextState* obj, int idx)
    {
        ...
        return static_cast<int>(p - obj->TextA.Data);
    }

    static bool ImCharIsSeparatorW(unsigned int c)
    {
        static constexpr unsigned int separator_list[] =
        {
...
void ImGuiInputTextState::OnCharPressed(unsigned int c)
{
    ...
    stb_textedit_text(this, Stb, utf8, static_cast<int>(strlen(utf8)));
    CursorFollow = true;
    CursorAnimReset();
}
    ...
    ImGuiInputTextCallback callback, void* user_data, bool input_source_is_clipboard)
{
        ...
        static_cast<ImGuiInputTextFlags>(ImGuiInputTextFlags_LocalizeDecimalPoint))))
    {
        ...
        ImGuiContext& g = *ctx;
        const unsigned c_decimal_point = g.PlatformIO.Platform_LocaleDecimalPoint;
        if (flags & (ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsScientific | static_cast<
    ...
    ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    // Multiline will not work with left-trimming
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs || init_reload_from_user_buf)
    {
        ...
        state->TextLen = static_cast<int>(strlen(buf));
    ...
    if (g.ActiveId == id)
    {
        ...
        constexpr ImGuiKey always_owned_keys[] = {
            ImGuiKey_LeftArrow, ImGuiKey_RightArrow, ImGuiKey_Enter, ImGuiKey_KeypadEnter, ImGuiKey_Delete,
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        else if (is_paste)
        {
            if (const char* clipboard = GetClipboardText())
            {
                ...
                auto clipboard_filtered = static_cast<char*>(IM_ALLOC(clipboard_len + 1));
                int clipboard_filtered_len = 0;
                for (const char* s = clipboard; *s != 0;)
                {
                    unsigned int c;
                    int len = ImTextCharFromUtf8(&c, s, nullptr);
                    s += len;
    ...
    if (render_cursor || render_selection)
    {
...

        {
            ...
            const char* selmin_ptr = render_selection
        ...
        // Draw selection
        const auto draw_scroll = ImVec2(state->Scroll.x, 0.0f);
        if (render_selection)
        {
            ...
            for (const char* p = text_selected_begin; p < text_selected_end;)
            {
                ...
                {
                    p = static_cast<const char*>(memchr((void*)p, '\n', text_selected_end - p));
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            if (flags & ImGuiInputTextFlags_ElideLeft)
                draw_pos.x = ImMin(
                    draw_pos.x, frame_bb.Max.x - CalcTextSize(buf_display, nullptr).x - style.FramePadding.x);

            constexpr auto draw_scroll = /*state ? ImVec2(state->Scroll.x, 0.0f) :*/ ImVec2(0.0f, 0.0f);
            // Preserve scroll when inactive?
            ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, col, buf_display,
                buf_display_end, 0.0f, is_multiline ? nullptr : &clip_rect);
        }
    ...
    if (is_multiline)
    {
        ...
        g.NextItemData.ItemFlags |= static_cast<ImGuiItemFlags>(ImGuiItemFlags_Inputable) | ImGuiItemFlags_NoTabStop;
        EndChild();
...
void ImGui::SetNextItemOpen(bool is_open, ImGuiCond cond)
{
    ...
    g.NextItemData.OpenCond = static_cast<ImU8>(cond ? cond : ImGuiCond_Always);
}
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    const ImGuiItemFlags extra_item_flags = disabled_item
        ? static_cast<ImGuiItemFlags>(ImGuiItemFlags_Disabled)
        : ImGuiItemFlags_None;
    bool is_visible;
    if (span_all_columns)
    {
    ...
    // Automatically close popups
    if (pressed && (window->Flags & ImGuiWindowFlags_Popup) && !(flags & ImGuiSelectableFlags_NoAutoClosePopups) && (g.
    ...
    ImGuiMultiSelectFlags ms_flags)
{
    ...
    if (ms_flags & ImGuiMultiSelectFlags_BoxSelect2d)
            ...
            BoxSelectRectCurr.Max.x)
        {
            ...
            // FIXME-OPT: UnclipRect x coordinates could be intersection of Prev and Curr rect on X axis.
...
void ImGui::EndBoxSelect(const ImRect& scope_rect, ImGuiMultiSelectFlags ms_flags)
{
    ...
    // FIXME-MULTISELECT: Styling
    window->DrawList->AddRect(box_select_r.Min, box_select_r.Max, GetColorU32(ImGuiCol_NavCursor));
    // FIXME-MULTISELECT: Styling

    // Scroll
    const bool enable_scroll = (ms_flags & ImGuiMultiSelectFlags_ScopeWindow) && (ms_flags &
        ImGuiMultiSelectFlags_BoxSelectNoScroll) == 0;
    if (enable_scroll)
    {
...
ImGuiMultiSelectIO* ImGui::BeginMultiSelect(ImGuiMultiSelectFlags flags, int selection_size, int items_count)
{
    ...
    if (g.NavJustMovedToId != 0 && g.NavJustMovedToFocusScopeId == ms->FocusScopeId && g.NavJustMovedToHasSelectionData)
    {
        ...
        if ((ms->KeyMods & (ImGuiMod_Ctrl | ImGuiMod_Shift)) == 0 && (flags & (ImGuiMultiSelectFlags_NoAutoClear |
...
void ImGui::MultiSelectItemHeader(ImGuiID id, bool* p_selected, ImGuiButtonFlags* p_button_flags)
{
    ...
    if (p_button_flags != nullptr)
    {
        ...
        if ((!selected || (g.ActiveId == id && g.ActiveIdHasBeenPressedBefore)) && !(ms->Flags &
...
void ImGui::MultiSelectItemFooter(ImGuiID id, bool* p_selected, bool* p_pressed)
{
    ...
    const bool enter_pressed = pressed && (g.NavActivateId == id) && (g.NavActivateFlags &
        ImGuiActivateFlags_PreferInput);
    ...
    if (pressed && (!enter_pressed || !selected))
    {
        ...
        if (flags & (ImGuiMultiSelectFlags_BoxSelect1d | ImGuiMultiSelectFlags_BoxSelect2d))
            if (selected == false && !g.BoxSelectState.IsActive && !g.BoxSelectState.IsStarting && input_source ==
                ImGuiInputSource_Mouse && g.IO.MouseClickedCount[0] == 1)
    ...
    ImGuiSelectionUserData first_item, ImGuiSelectionUserData last_item)
{
    ...
    if (ms->IO.Requests.Size > 0 && first_item == last_item && (ms->Flags & ImGuiMultiSelectFlags_NoRangeSelect) == 0)
    {
        ...
        if (prev->Type == ImGuiSelectionRequestType_SetRange && prev->RangeLastItem == ms->LastSubmittedItem && prev->
            Selected == selected)
        {
    ...
    ImGuiSelectionRequest req = {
        ImGuiSelectionRequestType_SetRange, selected, static_cast<ImS8>(range_dir),
...
void ImGui::DebugNodeMultiSelectState(ImGuiMultiSelectState* storage)
{
    ...
    bool open = TreeNode((void*)static_cast<intptr_t>(storage->ID), "MultiSelect 0x%08X in '%s'%s", storage->ID,
...
ImGuiSelectionBasicStorage::ImGuiSelectionBasicStorage()
{
    ...
    PreserveOrder = false;
    UserData = nullptr;
    AdapterIndexToStorageId = [](ImGuiSelectionBasicStorage*, int idx) { return static_cast<ImGuiID>(idx); };
...
{
    int lhs_v = static_cast<const ImGuiStoragePair*>(lhs)->val_i;
    int rhs_v = static_cast<const ImGuiStoragePair*>(rhs)->val_i;
...
{
    auto it = static_cast<ImGuiStoragePair*>(*opaque_it);
    ImGuiStoragePair* it_end = _Storage.Data.Data + _Storage.Data.Size;
    if (PreserveOrder && it == nullptr && it_end != nullptr)
        ImQsort(_Storage.Data.Data, static_cast<size_t>(_Storage.Data.Size), sizeof(ImGuiStoragePair),
...
void ImGuiSelectionBasicStorage::ApplyRequests(ImGuiMultiSelectIO* ms_io)
{
    ...
    for (ImGuiSelectionRequest& req : ms_io->Requests)
    {
        ...
        {
            const int selection_changes = static_cast<int>(req.RangeLastItem) - static_cast<int>(req.RangeFirstItem) +
                1;
            ...
            else
            {
                ...
                for (int idx = static_cast<int>(req.RangeFirstItem); idx <= static_cast<int>(req.RangeLastItem); idx++,
                    selection_order += req.RangeDirection)
                    ImGuiSelectionBasicStorage_BatchSetItemSelected(this, GetStorageIdFromIndex(idx), req.Selected,
        ...
        0.0f)
    {
        ...
        display_draw_list->AddLine(bb.GetTL() + ImVec2(x_offset, y_offset), bb.GetTR() + ImVec2(-x_offset, y_offset),
            GetColorU32(tab_bar_focused
                ...
                : ImGuiCol_TabDimmedSelectedOverline), style.TabBarOverlineSize);
    }
    RenderNavCursor(bb, id);
